    ...
    initialize: function initialize() {
      ...
      this.model.on('change:sort', function () {
        ...
        if (sortStr !== sort.id || dir !== direction) {
          _this.sortWidget.store.dispatch(SortActions.setQuery(null));

          _this.sortWidget.store.dispatch(SortActions.setSort(sortStr, true));

          _this.sortWidget.store.dispatch(SortActions.setDirection(dir, true));
        }
      });
    },
  ...
  var FormWidget = BaseWidget.extend({
          ...
          if (value) {
            ...
            if (value.includes(',')) {
              ...
              newValue = entries.reduce(function (acc, entry) {
                return _objectSpread(_objectSpread({}, acc), process(entry));
              }, defaultValue);
            } else {
              newValue = _objectSpread(_objectSpread({}, defaultValue), process(value));
            }
    ...
    getDbSelectionFromUserData: function getDbSelectionFromUserData() {
      try {
        ...
        var hasTrueVal = false;
        var dbs = userData.defaultDatabase.reduce(function (acc, db) {
          if (db.value) {
            hasTrueVal = true;
          }
