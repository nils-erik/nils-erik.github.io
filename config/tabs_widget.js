 ...
 */
define(['underscore', 'marionette', 'bootstrap', 'hbs!js/widgets/tabs/templates/tabs_inner', 'hbs!js/widgets/tabs/templates/tabs_outer', 'hbs!js/widgets/tabs/templates/tabs_title'], function (_, Marionette, Bootstrap, innerTemplate, outerTemplate, titleTemplate) {
  var TabsWidget = Marionette.ItemView.extend({
    ...
    render: function render() {
      ...
      _.each(this.tabs, function (t) {
        $nav.append(titleTemplate({
          id: t.id,
          title: t.title,
          default: t.default
        }));
        $tab.append(innerTemplate({
          id: t.id,
          default: t.default
        }));
      }, this);
      ...
      _.each(this.tabs, function (t) {
        var self = this; // attaching the html of child widgets

        this.$('#' + t.id).append(t.widget.getEl());
        this.$('a[href="#' + t.id + '"]').on('show.bs.tab', function () {
    ...
    onHidden: function onHidden(tab) {
