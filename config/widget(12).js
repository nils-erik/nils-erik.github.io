define(['marionette', 'js/widgets/base/base_widget', 'js/components/api_response', 'js/components/json_response', 'js/components/api_request', 'js/components/api_query', 'js/mixins/dependon', 'hbs!js/widgets/citation_helper/templates/citation_helper_template', 'bootstrap', 'js/components/api_feedback', 'js/components/api_targets', 'analytics'], function (Marionette, BaseWidget, ApiResponse, JsonResponse, ApiRequest, ApiQuery, Dependon, CitationHelperTemplate, bs, ApiFeedback, ApiTargets, analytics) {
  ...
  var View = Marionette.ItemView.extend({
    ...
    libraryAdd: function libraryAdd() {
      // show loading view
      this.$('.submit-add-to-library').html('<i class="fa fa-spinner fa-pulse" aria-hidden="true"></i>');
      var data = {}; // we have the selected library in the model but only if there was a select event, so query DOM

      data.libraryID = this.model.get('libraryID');
      data.libname = this.model.get('libname');
      var bibcodes = this.$('input:checked').map(function () {
        return this.value;
      }).get();
  ...
  var CitationHelperWidget = BaseWidget.extend({
    ...
    libraryAddSubmit: function libraryAddSubmit(data) {
      ...
      name = data.libname; // this returns a promise
    ...
    getSuggestions: function getSuggestions(bibcodes) {
      var d = $.Deferred();
    ...
    processSuggestions: function processSuggestions(response) {
    ...
    renderWidgetForListOfBibcodes: function renderWidgetForListOfBibcodes(bibcodes, data) {
      this.reset();
      var request = new ApiRequest({
        target: ApiTargets.SERVICE_CITATION_HELPER,
        query: new ApiQuery({
