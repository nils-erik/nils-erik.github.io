define(['./base_graph', 'hbs!js/widgets/facet/graph-facet/templates/h-index-graph-legend', 'hbs!js/widgets/facet/graph-facet/templates/h-index-slider-window', 'marionette'], function (BaseGraphView, legendTemplate, sliderWindowTemplate, Marionette) {
  var HIndexGraphView = BaseGraphView.extend({
    legendTemplate: legendTemplate,
    xAxisClassName: 'h-index-x-axis-title',
    addToOnRender: function addToOnRender() {
      // show the h index
      if (this.hIndex) {
    ...
    buildGraph: function buildGraph() {
      ...
      chart = d3.select(this.el).select('.chart').attr('width', this.fullWidth).attr('height', this.fullHeight).attr('aria-label', this.yAxisTitle + ' graph');
      ...
      if (data.length >= 40) {
        ...
        d.enter().append('circle').attr('class', 'dot').attr('r', 2).classed('refereed', function (d) {
          if (d.refereed) {
            return true;
          }
        }).attr('cx', function (d) {
      ...
      if (this.hIndex) {
        // first h index line
        this.innerChart.append('line').classed({
          'h-index': true,
          'h-index-1': true
        }).attr('x1', x(this.hIndex.x)).attr('y1', y(minVal - 1)).attr('x2', x(this.hIndex.x)).attr('y2', y(this.hIndex.y)); // second h index line

        this.innerChart.append('line').classed({
          'h-index': true,
          'h-index-2': true
        }).attr('x1', x(xLabels[0] - 1)).attr('y1', y(this.hIndex.y)).attr('x2', x(this.hIndex.x)).attr('y2', y(this.hIndex.y));
      }

      this.innerChart.append('text').attr('class', 's-label').attr('x', this.width / 2 - 20).attr('y', 180).text(Marionette.getOption(this, 'xAxisTitle'));
    ...
    toggleScale: function toggleScale(e) {
      var v = $(e.target).attr('value');
      this.currentScale = v;
      this.graphChange();
    },
    graphChange: function graphChange(val) {
      var data;
      var max;
      var x;
      var y;
      ...
      if (this.currentScale === 'log') {
        var logFormat = function logFormat(d) {
      ...
      if (this.hIndex) {
        ...
        this.innerChart.select('.h-index-1').transition().attr('x1', x(this.hIndex.x)).attr('x2', x(this.hIndex.x)).attr('y2', y(this.hIndex.y)); // second h index line
    ...
    addSliderWindows: function addSliderWindows() {
      this.$('.slider-data').html(sliderWindowTemplate({
        pastTenseTitle: this.pastTenseTitle
      }));
    },
    triggerGraphChange: function triggerGraphChange(update) {
      ...
      if (_.isArray(data)) {
        ...
        var a = $first.val(); // a < min, set to min

        a = a < min ? min : a; // a > max, set to b
