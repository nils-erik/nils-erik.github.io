define(['d3', 'marionette', 'js/widgets/base/base_widget', 'js/components/api_request', 'hbs!js/widgets/bubble_chart/templates/container', 'hbs!js/widgets/bubble_chart/templates/tooltip', 'js/components/api_targets', 'js/components/api_query_updater', 'js/components/api_query'], function (d3, Marionette, BaseWidget, ApiRequest, ContainerTemplate, TooltipTemplate, ApiTargets, ApiQueryUpdater, ApiQuery) {
  ...
  var BubbleView = Marionette.ItemView.extend({
    ...
    cacheVals: function cacheVals() {
      this.cache = {};
      this.cache.svg = d3.select(this.$('svg.bubble-chart-svg')[0]).append('g').attr('transform', 'translate(' + this.config.margin.left + ',' + this.config.margin.top + ')');
    ...
    setScales: function setScales() {
      ...
      if (this.model.get('journalNames').length) {
        this.scales.journalScale = d3.scale.ordinal().domain(this.model.get('journalNames')).range(['hsla(282, 80%, 52%, 1)', 'hsla(1, 80%, 51%, 1)', 'hsla(152, 80%, 40%, 1)', 'hsla(193, 80%, 48%, 1)', 'hsla(220, 80%, 56%, 1)', 'hsla(0, 0%, 20%, 1)']);
      } // x scale
      ...
      if (that.model.get('xValue') === 'date') {
        ...
        this.scales.xScale = d3.time.scale().domain(newDateRange).range([0, that.config.width]);
        ...
        if (that.model.get('xScale') === 'log') {
          this.scales.xScale = d3.scale.log().domain(xExtent).range([0, that.config.width]);
        } else {
          this.scales.xScale = d3.scale.linear().domain(xExtent).range([0, that.config.width]);
        }
      }
      ...
      if (this.model.get('yScale') === 'log') {
        this.scales.yScale = d3.scale.log().domain(yExtent).range([that.config.height, 0]);
      } else {
        this.scales.yScale = d3.scale.linear().domain(yExtent).range([that.config.height, 0]);
      } // radius scale


      if (that.model.get('radius') == 'year') {
        this.scales.radiusScale = d3.scale.linear().domain(d3.extent(data, function (d) {
    ...
    rearrangeGraph: function rearrangeGraph() {
      ...
      yAxis = d3.svg.axis().scale(this.scales.yScale).orient('left').ticks(10).tickFormat(d3.format('s')); // X Axis
