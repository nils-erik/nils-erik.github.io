define(['js/page_managers/controller', 'js/page_managers/three_column_view', 'hbs!js/page_managers/templates/results-page-layout', 'js/mixins/side_bar_manager', 'jquery', 'utils'], function (PageManagerController, PageManagerView, PageManagerTemplate, SideBarManagerMixin, $, utils) {
  var PageManager = PageManagerController.extend({
    ...
    activate: function activate() {
      ...
      ps.subscribe(ps.START_SEARCH, _.bind(this.onStartSearch, this)); // calls the sideBarManager mixin init handler
    ...
    setUpIntercepts: function setUpIntercepts() {
      ...
      var interceptRequests = function interceptRequests(widget) {
        ...
        wps.publish = function (ev) {
          ...
          if (ev === wps.DELIVERING_REQUEST && !_this.getSidebarState()) {
      ...
      widgets.map(function (k) {
        return interceptRequests(_this.widgets[k]);
      });
    },
